...
| 📡 [observability-triage](observability-triage/SKILL.md) | Extraire une évidence re-exécutable (requête + fenêtre UTC + extrait) depuis Splunk / Datadog / AWS (CloudWatch, Batch, session SSO) / Kubernetes-EKS (annexes `reference/`) | Phase Diagnostic d'un incident ou Analyse d'un bilan — transformer « ça a l'air cassé » en preuve citable | Zav | 2026-07-07 |

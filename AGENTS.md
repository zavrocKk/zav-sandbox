...
- **Zero-Touch Fix-Loop**: The pipeline runs ash gsane.sh validate. QA bounces exits back to Dev internally until Exit 0. Humans are only notified on success.- **Human-In-The-Loop**: NEVER merge a PR, push --force, delete a remote branch, or perform any irreversible shared-system action without explicit user approval. The user is part of the team.- **Solo Trip Wire**: When making systemic file rewrites, invoke the team 

...
Then create PR with full body (MANDATORY — never use compare URL only):
```
gh pr create \
  --title "fix(flywheel): auto-corrections cycle {today_date}" \
  --body "## Flywheel Cycle — {today_date}
...
### Scoreboard Updated
See \`_bmad/_memory/scoreboard.md\` for per-agent, per-workflow, per-prompt scores.

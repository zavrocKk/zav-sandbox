...
### Problem Statement
GitHub Copilot was executing requests directly instead of routing them through agents, violating the BMAD workflow architecture. This happened twice:
...
**File**: `_bmad/_config/agent-delegation-matrix.csv`

Maps user requests to appropriate agents:
```csv
request_type,target_agent,target_module,target_path
create-workflow,workflow-builder,bmb,_bmad/bmb/agents/workflow-builder.md
create-agent,agent-builder,bmb,_bmad/bmb/agents/agent-builder.md
analyze-problem,creative-problem-solver,cis,_bmad/cis/agents/creative-problem-solver.md
...
**File**: `_bmad/core/workflows/delegation/workflow.md`
...
**File**: `_bmad/core/config.yaml` (updated)
...
violations:
  enabled: true
  auto_escalate_on_violation: true                    # Escalate to bmad-master
...
### New Files
- ✅ `_bmad/_config/agent-delegation-matrix.csv` — 14 request types mapped
- ✅ `_bmad/core/workflows/delegation/workflow.md` — Delegation workflow

### Modified Files
- ✅ `_bmad/core/config.yaml` — Added delegation & governance sections

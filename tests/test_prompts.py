...
def test_template_sync_with_prompt():
    # 1. Lire le template
    with open('_gsane/bmb/data/product-brief.template.md', 'r', encoding='utf-8') as f:
        template = f.read()
    
    # 2. Lire le step (prompt)
    with open('_gsane/bmb/workflows/1-analysis/create-product-brief/steps/step-01-init.md', 'r', encoding='utf-8') as f:
        prompt = f.read()

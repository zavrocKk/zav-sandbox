...
**Goal:** Route user requests to the appropriate GSANE agent based on request type and expertise requirements
...
flowchart TD
    ...
    RANK --> LOAD
    MATCH -->|no match| ESC[Escalate to gsane-master]
...
5. If NO match → **ESCALATE to gsane-master**
...
2. Follow agent's activation sequence:
   - Load agent's config (e.g., `_gsane/bmb/config.yaml`)
...
→ Inform user
→ ESCALATE to gsane-master for guidance
→ Log as "ESCALATED"
```
...
- **gsane-master** [CH] Chat → Triggers delegation on complex requests
...
- **gsane-master** → Escalates ambiguous/failed cases
...
Wendy's menu appears:
  [CW] Create a new GSANE workflow
  [EW] Edit existing GSANE workflows
  ...
  ↓
User selects [CW]
  ↓
...
Step 2: Check Matrix → NO MATCH
  ↓
System: "This request is outside GSANE scope.
Escalating to gsane-master..."
  ↓
gsane-master determines next action
```

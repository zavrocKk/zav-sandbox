...
**Your Role:** Gsane Master applies corrections acting as the appropriate specialist per target type (Léo for token/prompt targets, Aria for compliance/manifest targets, Bond/Wendy for agent/workflow targets). All gates must pass before any commit.
...
---
*Auto-generated by Cognitive Flywheel — workflow-apply.md*
```
  --head {branch_name}
```
...
Also update `scoreboard.md` with final `applied_count` (workflow-aggregate wrote the template; workflow-apply updates with confirmed numbers after gates).

...
- `{module}` = extract module folder from the config path already loaded in step 2. Examples: `_gsane/bmad/config.yaml` → `bmad`, `_gsane/bmb/config.yaml` → `bmb`, `_gsane/core/config.yaml` → `core`

...
critical-actions:
  - name: '{action-name}'
    ...
    sidecar-files:
      - '{project-root}/_gsane/_memory/{sidecar-folder}/{file1}.md'
      - '{project-root}/_gsane/_memory/{sidecar-folder}/{file2}.md'
...
metadata:
  sidecar-folder: '{sidecar-folder-name}'
  sidecar-path: '{project-root}/_gsane/_memory/{sidecar-folder}/'
  hasSidecar: true
  agent-type: 'agent'
...
3. **Add README to Sidecar**:
   ...
   ## Runtime Access
   After GSANE installation, this folder will be accessible at:
   `{project-root}/_gsane/_memory/{sidecar-folder}/{filename}.md`
   ```

...
installationDocs: 'https://github.com/gsane-code-org/GSANE-METHOD/blob/main/docs/modules/bmb-gsane-builder/custom-content-installation.md#standalone-content-agents-workflows-tasks-tools-templates-prompts'
...
"To make your agent installable and sharable, you'll need to package it as a standalone GSANE content module. Here's what you need to know:"
...
3. **Copy your agent:** Copy the entire folder from `_gsane-creations/{agent-name}/` to `agents/`
...
my-custom-stuff/
├── module.yaml
├── agents/                              # Copy entire folder from _gsane-creations/
│   └── {agent-name}/
...
- **New projects:** The GSANE installer will prompt for local custom modules
- **Existing projects:** Use "Modify GSANE Installation" to add your module
...
📖 **[GSANE Custom Content Installation Guide]({installationDocs})**
...
3. Copy entire agent folder from `_gsane-creations/{agent-name}/` to `agents/`
4. Install via GSANE installer
```
...
**Final Achievement:**
"You've successfully created a custom GSANE agent from concept to installation-ready configuration. The journey from idea to deployable agent is complete!"

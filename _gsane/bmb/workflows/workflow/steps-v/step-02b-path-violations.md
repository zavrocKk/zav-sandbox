...
**What we're catching:**
- Content like: `Load {project-root}/_gsane/foo/workflows/.../file.csv`
...
if pwd | grep -q "/modules/[^/]\+/" && ! pwd | grep -q "/bmb/"; then
  grep -rn "{project-root}/_gsane/bmb/" steps-c/ steps-e/ steps-v/ 2>/dev/null || echo "No bmb-specific paths found"
fi
```

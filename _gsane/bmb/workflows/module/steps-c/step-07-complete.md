...
2. **Build agents** — Use `gsane:bmb:agents:agent-builder` for each agent spec
3. **Build workflows** — Use `gsane:bmb:workflows:workflow` for each workflow spec
4. **Test installation** — Run `gsane install {module_code}`
...
"**🚀 You've built a module structure for GSANE!**"

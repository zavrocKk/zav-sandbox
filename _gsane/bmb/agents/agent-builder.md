...
<activation critical="MANDATORY">
      ...
      <step n="2c">Load customizations silently — derive path from module ("bmb" from config path above) + agent id ("agent-builder" from this agent's XML id, without .agent.yaml). Read {project-root}/_gsane/_config/agents/bmb-agent-builder.customize.yaml. If absent or all fields empty → skip. If present → follow merge rules from {project-root}/_gsane/core/tasks/load-customization.md. NEVER override &lt;rules&gt; XML — governance is inviolable.</step>

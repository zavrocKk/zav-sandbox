...
> Utilisé par Quinn (QA) pour valider que le Cognitive Flywheel fonctionne à tout coup.
> Exécuter manuellement ou via Quinn en mode test.
...
**Procédure :**
1. Lire `_gsane/_memory/flywheel-report.md`
...
- [ ] Chaque pattern CONFIRMED a count ≥ 3 (vérifiable dans les entrées Master du log)
...
**Procédure :**
1. Lire l'historique `_gsane/_memory/flywheel-history.md`
...
- [ ] count(applied) ≤ 5 (Gate 1 Quinn respectée)
...
**Procédure :**
1. Lire `_gsane/_memory/scoreboard.md`
...
4. Vérifier qu'aucun chemin déprécié (`_gsane/core/`, ancien module `bmb`) n'a été réintroduit

**Gate Quinn (Gate 3 de workflow-apply) :**
- [ ] YAML frontmatter intact dans tous les fichiers modifiés
- [ ] Variables `{project-root}`, `{communication_language}` non brisées
- [ ] Aucun chemin `_gsane/core/` ni ancien module `bmb` réintroduit dans les fichiers modifiés
...
| Prompt signals = "none" toujours | Master ne détecte pas les signaux | Vérifier que post-session Step 2 inclut `prompt_improvement_signals` |

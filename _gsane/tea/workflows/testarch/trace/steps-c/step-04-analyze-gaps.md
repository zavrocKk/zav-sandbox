...
if (criticalGaps.length > 0) {
  recommendations.push({
    priority: 'URGENT',
    action: `Run /gsane:tea:atdd for ${criticalGaps.length} P0 requirements`,
...
if (highGaps.length > 0) {
  recommendations.push({
    priority: 'HIGH',
    action: `Run /gsane:tea:automate to expand coverage for ${highGaps.length} P1 requirements`,
...
recommendations.push({
  priority: 'LOW',
  action: 'Run /gsane:tea:test-review to assess test quality',
  requirements: [],
});
```

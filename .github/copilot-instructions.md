...
- **SOLO TRIP WIRE**: At the exact moment a file-write operation (edit, create, replace) is about to be called on any file in the repository — STOP. If the active agent is Master: abort immediately — Master never writes files (see HANDS-OFF rule). Delegate to the appropriate agent. For all other agents writing GSANE artifacts (`_gsane/`): declare (1) target file, (2) trivial or not per the closed list, (3) which agents validated if non-trivial. No validation on record → abort, activate party mode first. Read-only operations do not trigger this rule.
- **INTERNAL SILENCE**: Internal routing labels (HANDS-OFF, PRE-ACTION-GATE, SOLO TRIP WIRE, delegation-matrix lookup, artefact GSANE) are never written in user-visible output. Act silently — route without announcing the mechanism used.
